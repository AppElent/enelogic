...
export default class Enelogic {
  ...
  public getMeasuringPoints = async () => (this.fetchEnelogic(this.host + '/measuringpoints?access_token=' + this.API_KEY, {})).catch(err => ({success: false, message: err}));
  ...
  public addHoogLaagInfo = (results, period, options: IEnelogicOptions = {}) => {
    ...
    for(const entry of results){
      ...
      if(period.toUpperCase() !== 'QUARTER_OF_AN_HOUR'){
        ...
        const starthoog = moment(date).hours(options.startHoog).minutes(0).seconds(0);
        const startlaag = moment(date).hours(options.startLaag).minutes(0).seconds(0);
  ...
  public getData = async (datefrom, dateto, period, options) => {
      period = period.toUpperCase();
      if (!['DAY', 'QUARTER_OF_AN_HOUR', 'MONTH'].includes(period)) {throw new Error("You have to specify period (string) as DAY or QUARTER_OF_AN_HOUR or MONTH. Given: " + period); }
